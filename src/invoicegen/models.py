...
class Jobline(BaseModel):
    ...
    def str_maxlen(cls: Any, value: Any) -> str:
    ...
    def validate_decimals(cls: Any, value: Any, info: ValidationInfo) -> Decimal:
        ...
        if isinstance(value, float):
            raise ValueError(
                f"{info.field_name} must not be a float, please use string, decimal, or int"
            )

        # Make sure input is a string, integer, or decimal
        if not isinstance(value, (str, int, Decimal)):
            raise ValueError(
                f"{info.field_name} must be a string, int, or decimal, got {type(value).__name__}"
            )
        ...
        if isinstance(value, str):
            ...
            if not len(value):
                raise ValueError(
                    f"{info.field_name} is empty after removing symbols; provide a number"
                )

...
import re
from pydantic import BaseModel, field_validator, model_validator, ValidationError, ValidationInfo
...
class Jobline(BaseModel):
    ...
    @field_validator("address", "unit", "description", mode="before")
    def ensure_nonempty_str(cls, value, info: ValidationInfo) -> str:
        ...
        if len(value) < 1:
            raise ValueError(f"{info.field_name} is empty, expected a value")
        
        return value
    
    @field_validator("description", mode="before")
    def str_maxlen(cls, value) -> str:
        # Previous validator made sure it was non-empty and stripped
        # Check if description is less than 2000 characters long
        if len(value) > 2000:
            raise ValueError(f"Description is too long (max 2000 chars)")
    ...
    def validate_date(cls, value) -> date:
        # Make sure it is a string
        if not isinstance(value, str):
            raise ValueError(f"Date must be a string in MM/DD/YYYY format, got {type(value).__name__}")
        ...
        try:
            ...
            raise ValueError(f"Invalid calendar date '{value}' (MM/DD/YYYY)")
        
        return parsed
    
    @field_validator("qty", "rate", mode="before") 
    def validate_decimals(cls, value, info: ValidationInfo) -> Decimal:
        # We do not want any floats for precision
        if isinstance(value, float):
            raise ValueError(f"{info.field_name} must not be a float, please use string, decimal, or int")
        
        # Make sure input is a string, integer, or decimal
        if not isinstance(value, (str, int, Decimal)):
            raise ValueError(f"{info.field_name} must be a string, int, or decimal, got {type(value).__name__}")
        ...
        if isinstance(value, str):
            ...
            if not len(value):
                raise ValueError(f"{info.field_name} is empty after removing symbols; provide a number")
            
            # Make sure pattern is a decimal number
            if not re.fullmatch(r"^[0-9]+(\.[0-9]+)?$", value):
                raise ValueError(f"{info.field_name} must be digits.decimals (e.g. 1.5, 65)")
        ...
        if dec < 0:
            raise ValueError(f"{info.field_name} must be at least 0: got {dec}")
    ...
    def validate_row(cls, value) -> int:
        if not isinstance(value,int):
            raise ValueError(f"Source row must be an int, got {type(value).__name__}, how did this happen?")
        if value < 2:
